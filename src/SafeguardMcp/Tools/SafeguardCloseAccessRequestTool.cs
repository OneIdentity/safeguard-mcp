...
internal sealed class SafeguardCloseAccessRequestTool(ISafeguardSession session)
{
    ...
    [Description("Close a Safeguard access request by dispatching to the correct sub-endpoint "
        ...
        + "RequestCheckedIn / Terminated / PendingReview / PendingAccountSuspended, where the V4 "
        + "controller's Close attribute is PolicyAdmin-only; for those the tool returns a diagnostic "
        + "naming the state rather than calling Close as a non-admin (which would 403). "
        + "PolicyAdmin acting on someone else's request only dispatches Close from PendingReview "
        + "(this diverges from safeguard-ps, which sends Close unconditionally for admins, in favour "
        + "of the V4 controller's documented \"PolicyAdmin permission and request must be in "
        + "PendingReview state\" contract). "
